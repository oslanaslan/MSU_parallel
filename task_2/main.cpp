...
int main(int argc, char **argv) {
	...
	prog_time = (unsigned int)(time(NULL) - start_time);
	long long int max_prog_time = 0;
	MPI_Reduce(&prog_time, &max_prog_time, 1, MPI_LONG_LONG, MPI_MAX, MASTER, MPI_COMM_WORLD);
	if (rank == MASTER) {
		max_prog_time = max_prog_time > prog_time ? max_prog_time : prog_time;
