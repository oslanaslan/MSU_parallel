...
Solver::Solver(Computer &computer, double eps) {
    MPI_Init();
    MPI_Comm_size(MPI_COMM_WORLD, &this->thread_count);
    ...
    std::fill(this->multiplied_error, this->multiplied_error + this->grid_size, 0);
    std::fill(this->diff_w, this->diff_w + this->grid_size, 0);
}
...
void Solver::solve() {
    ...
    do
    {
        for (int64_t i = 0; i < this->grid[this->current_rank].size(); i++) {
            int64_t index = this->grid[this->current_rank][i];
        ...
        this->synchronize(this->error_vec);
        ...
        this->synchronize(this->multiplied_error);
        ...
        this->share_tolerance(tolerance);
...
void Solver::synchronize(double* vector) {
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Reduce(vector, vector, this->grid_size, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Bcast(vector, this->grid_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
}

void Solver::share_tolerance(double& tolerance) {
